...
async function loadPron(){try{const r=await fetch("data/pronunciations.json?v=3.29.5",{cache:"no-cache"});const j=await r.json();lookupPronunciations=j?.words||{};}catch(e){} const q=new URLSearchParams(location.search).get("word");if(q)runLookup(q);}
